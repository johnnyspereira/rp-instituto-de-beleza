-- Restore universal voucher/pack lookup and PIN validation even when migration 054
-- was skipped or PostgREST lost the function from its schema cache.
...
WHERE NULLIF(BTRIM(pin_code), '') IS NULL;
...
BEGIN
  NEW.code := UPPER(COALESCE(NULLIF(BTRIM(NEW.code), ''),
    'PCK-' || SUBSTRING(MD5(RANDOM()::TEXT || CLOCK_TIMESTAMP()::TEXT) FROM 1 FOR 10)));
  NEW.pin_code := COALESCE(NULLIF(BTRIM(NEW.pin_code), ''),
...
BEGIN
  ...
  IF FOUND THEN
    SELECT catalog.name INTO v_pack_name
    FROM finance_pack_catalog catalog
    WHERE catalog.id = v_pack.pack_id;

    SELECT COALESCE(SUM(total_sessions), 0), COALESCE(SUM(remaining_sessions), 0)
      INTO v_total, v_remaining
    FROM finance_client_pack_balances
    WHERE client_pack_id = v_pack.id;
...
BEGIN
  ...
  FROM clinic_appointments WHERE id = p_appointment_id FOR UPDATE;
  ...
  IF FOUND THEN
    ...
    ELSE
      ...
      v_reserved_amount := LEAST(v_appointment.price, v_voucher.current_balance - v_other_reserved);
  ...
  FROM finance_appointment_benefits
  WHERE client_pack_balance_id = v_balance.id
    AND status = 'reserved'
    AND appointment_id <> p_appointment_id;
...
REVOKE EXECUTE ON FUNCTION lookup_finance_benefit_code(TEXT) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION reserve_appointment_benefit_code(UUID, TEXT, TEXT) FROM PUBLIC;
