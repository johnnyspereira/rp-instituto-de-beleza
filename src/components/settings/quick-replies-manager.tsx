...
export function QuickRepliesManager() {
  ...
  const save = useCallback(async () => {
    if (!draft) return;
    if (!draft.title.trim()) {
      toast.error('Give the internal template a name.');
      return;
    }
    const payload =
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        toast.error(data.error ?? "Couldn't save the internal template.");
        return;
      }
      toast.success(
        draft.id ? 'Internal template updated.' : 'Internal template created.'
      );
      setDraft(null);
      await load();
    } catch {
      toast.error("Couldn't save the internal template.");
    } finally {
      setSaving(false);
    }
  }, [draft, load]);
  ...
  const remove = useCallback(
    async (id: string) => {
      if (!window.confirm('Delete this internal template?')) return;
      const res = await fetch(`/api/quick-replies/${id}`, { method: 'DELETE' });
      if (!res.ok) {
        toast.error("Couldn't delete the internal template.");
        return;
      }
      await load();
    },
  ...
  return (
    <div>
      <SettingsPanelHead
        ...
        description="Reusable CRM templates that do not go to Meta for approval. They send through the connected QR session like a normal WhatsApp Web message."
        action={
      ...
      {loading ? (
        ...
        <p className="border-border text-muted-foreground rounded-lg border border-dashed py-10 text-center text-sm">
          No internal templates yet. Create one to reuse it across
          conversations.
        </p>
      ) : (
      ...
      <Dialog open={!!draft} onOpenChange={(o) => !o && setDraft(null)}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              {draft?.id ? 'Edit internal template' : 'New internal template'}
            </DialogTitle>
          </DialogHeader>
          {draft && (
