...
//
// Role-gating
//   Settings navigation now exposes this tab only to admin+. The
//   mutation buttons are still wrapped in `<RequireRole min="admin">`
//   and the API/RPC layer double-checks the role, so direct calls are
//   blocked too.
// ============================================================
...
export function MembersTab() {
  ...
  const roleCounts = members.reduce(
    ...
    { owner: 0, admin: 0, agent: 0, viewer: 0 } satisfies Record<
      AccountRole,
      number
    >
  );
  ...
  async function handleProfessionalChange(
    ...
    const next: Member = {
      ...
      professional_title:
        patch.professional_title === undefined
          ...
          : patch.professional_title,
      professional_color:
        patch.professional_color ?? member.professional_color ?? '#7c3aed',
    };

    setPendingMemberAction(member.user_id);
    setMembers((prev) =>
      prev.map((m) => (m.user_id === member.user_id ? next : m))
    );
    ...
    try {
      const res = await fetch(`/api/account/members/${member.user_id}`, {
        ...
        body: JSON.stringify({
          professional: {
            is_professional: next.is_professional ?? false,
            title: next.professional_title ?? '',
            color: next.professional_color ?? '#7c3aed',
          },
        }),
      });
      if (!res.ok) {
        setMembers((prev) =>
          prev.map((m) => (m.user_id === member.user_id ? previous : m))
        );
        const payload = await res.json().catch(() => ({}));
        toast.error(payload.error || t('professionalUpdateFailed'));
        return;
      }
      toast.success(t('professionalUpdatedToast'));
    } catch (err) {
      setMembers((prev) =>
        prev.map((m) => (m.user_id === member.user_id ? previous : m))
      );
      console.error('[MembersTab] professional change error:', err);
      toast.error('Could not reach the server');
    } finally {
      setPendingMemberAction(null);
    }
    ...
    <section className="animate-in fade-in-50 space-y-6 duration-200">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
        <MemberSummaryCard
          ...
          detail={t('summary.totalDesc')}
        />
        <MemberSummaryCard
          icon={Crown}
          label={tRoles('owner')}
          value={roleCounts.owner}
          detail={t('summary.ownerDesc')}
        />
        <MemberSummaryCard
          icon={ShieldCheck}
          label={tRoles('admin')}
          value={roleCounts.admin}
          detail={t('summary.adminDesc')}
        />
        <MemberSummaryCard
          icon={UserCog}
          label={tRoles('agent')}
          value={roleCounts.agent}
          detail={t('summary.agentDesc')}
        />
        <MemberSummaryCard
          icon={BriefcaseBusiness}
          label={t('summary.professionals')}
          value={professionalCount}
          detail={t('summary.professionalsDesc')}
        />
        <MemberSummaryCard
          icon={Eye}
          label={t('summary.invites')}
          value={activeInviteCount}
          detail={t('summary.invitesDesc')}
        />
      </div>
        ...
        <CardHeader className="border-border border-b">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <CardDescription>{t('rosterDesc')}</CardDescription>
            </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-col gap-1 sm:items-end">
                    ...
                    <span className="text-muted-foreground max-w-48 text-left text-[11px] leading-4 sm:text-right">
                      {t(`roleHints.${member.role}`)}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 sm:justify-end">
                    <Button
                      ...
                      onClick={() =>
                        handleProfessionalChange(member, {
                    ...
                    <Input
                      ...
                      onBlur={(event) =>
                        handleProfessionalChange(member, {
                          professional_title: event.currentTarget.value,
                        })
                      }
                    ...
                    <input
                      type="color"
                      value={member.professional_color ?? '#7c3aed'}
                      disabled={!canManageMembers || isBusy}
                      onChange={(event) =>
                        handleProfessionalChange(member, {
                          professional_color: event.currentTarget.value,
                        })
                      }
                      aria-label={t('professionalColor')}
                      className="border-border bg-background h-9 w-10 rounded-md border p-1"
                    />
                  </div>
