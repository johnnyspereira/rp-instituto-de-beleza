...
const VARIABLE_COMMANDS = [
  ...
  { token: 'message.text', labelKey: 'messageText' },
  { token: 'conversation.id', labelKey: 'conversationId' },
  { token: 'automation.name', labelKey: 'automationName' },
  { token: 'trigger.event', labelKey: 'triggerEvent' },
] as const;

function appendVariable(value: string, token: string): string {
  const suffix = `{{ ${token} }}`;
  if (!value.trim()) return suffix;
  const needsSpace = !/[\s\n]$/.test(value);
  return `${value}${needsSpace ? ' ' : ''}${suffix}`;
}

function VariableCommandBar({
  onInsert,
  t,
}: {
  ...
  return (
    <div className="border-border bg-background/60 mt-2 rounded-md border p-2">
      ...
      <div className="mt-2 flex flex-wrap gap-1.5">
        {VARIABLE_COMMANDS.map((item) => (
          <button
            key={item.token}
            type="button"
            onClick={() => onInsert(item.token)}
            title={t('variables.insert', { token: item.token })}
            className="border-border bg-muted text-muted-foreground hover:border-primary/50 hover:bg-primary/10 hover:text-primary inline-flex h-7 items-center gap-1 rounded-md border px-2 text-[11px] font-medium transition-colors"
          >
            <Plus className="h-3 w-3" />
            {t(`variables.items.${item.labelKey}`)}
          </button>
        ))}
      </div>
    </div>
...
function StepEditor({
  ...
  switch (step.step_type) {
    case 'send_message': {
      const messageText = (cfg.text as string) ?? '';
      return (
        <FieldBlock label={t('config.messageText')}>
          ...
          <VariableCommandBar
            t={t}
            onInsert={(token) =>
              set({ text: appendVariable(messageText, token) })
            }
          />
        </FieldBlock>
      );
    ...
    case 'update_contact_field': {
      const updateValue = (cfg.value as string) ?? '';
      return (
        <>
          ...
          <FieldBlock label={t('config.valueLabel')}>
            ...
            <VariableCommandBar
              t={t}
              onInsert={(token) =>
                set({ value: appendVariable(updateValue, token) })
              }
            />
          </FieldBlock>
        </>
    ...
    case 'create_deal': {
      const dealTitle = (cfg.title as string) ?? '';
      return (
        <>
          <DealPipelineFields
          ...
          <FieldBlock label={t('config.titleLabel')}>
            <Input
              ...
              placeholder={t('config.placeholderTitle')}
              className="bg-muted text-foreground"
            />
            <VariableCommandBar
              t={t}
              onInsert={(token) =>
                set({ title: appendVariable(dealTitle, token) })
              }
            />
          </FieldBlock>
          <FieldBlock label={t('config.valueLabel')}>
        ...
        <>
          ...
          <FieldBlock label={t('config.bodyTemplateLabel')}>
            ...
            <VariableCommandBar
              t={t}
              onInsert={(token) =>
                set({ body_template: appendVariable(bodyTemplate, token) })
              }
            />
          </FieldBlock>
        </>
...
function previewFor(
  ...
  switch (step.step_type) {
    ...
    case 'condition':
      return t('preview.condition', {
        subject: String(step.step_config.subject ?? '?'),
      });
    case 'send_webhook':
