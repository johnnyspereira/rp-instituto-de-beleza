...
function pushText(lines: string[], value: string | undefined): void {
...
/**
 * WhatsApp Web QR sessions do not expose the same official interactive
 * buttons/list send API as Meta Cloud API. For QR delivery, keep the
 * message useful by flattening the payload into a normal text message.
 */
export function interactivePayloadToText(
  ...
  pushText(lines, payload.header);
  pushText(lines, payload.body);
  pushText(lines, payload.footer);
  ...
  if (payload.kind === 'buttons') {
    ...
    payload.sections.forEach((section) => {
      ...
      if (title) optionLines.push(title);
