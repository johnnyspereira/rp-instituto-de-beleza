...
/**
 * Resolve a WhatsApp chat id to an actual E.164-like phone number.
 *
 * Newer WhatsApp accounts can be represented by an opaque `@lid` id. A LID
 * must never be stored as a customer phone: it is only accepted when
 * WhatsApp also supplied the corresponding `@c.us` phone id.
 */
export function phoneFromWhatsAppJid(
  jid: string,
  mappedPhoneJid?: string | null
): string {
  const direct = jid.trim().toLowerCase();
  const mapped = mappedPhoneJid?.trim().toLowerCase() ?? '';
  const phoneJid = direct.endsWith('@c.us')
    ? direct
    : direct.endsWith('@lid') && mapped.endsWith('@c.us')
      ? mapped
      : '';
  if (!phoneJid) return '';

  const phone = normalizePhone(phoneJid.slice(0, -'@c.us'.length));
