...
describe('phoneFromWhatsAppJid', () => {
  it('extracts a real phone from a c.us WhatsApp id', () => {
    expect(phoneFromWhatsAppJid('351935864343@c.us')).toBe('351935864343');
  });

  it('uses the mapped phone id for an opaque LID', () => {
    expect(
      phoneFromWhatsAppJid('256530024804567@lid', '351935864343@c.us')
    ).toBe('351935864343');
  });

  it('never treats an unmapped LID as a customer phone', () => {
    expect(phoneFromWhatsAppJid('256530024804567@lid')).toBe('');
    expect(
      phoneFromWhatsAppJid('256530024804567@lid', '998877665544@lid')
    ).toBe('');
  });

  it('rejects groups, newsletters and invalid phone ids', () => {
    expect(phoneFromWhatsAppJid('351935864343@g.us')).toBe('');
    expect(phoneFromWhatsAppJid('123@c.us')).toBe('');
  });
});
