...
describe('update_contact_field — custom fields', () => {
  ...
  it('interpolates contact and trigger fields into the custom value', async () => {
    h.state.owned = {
      id: 'c1',
      name: 'Ana Silva',
      phone: '+351900000000',
    ...
    h.state.steps = [
      customStep(
        'custom:cf1',
        '{{ contact.name }} | {{ contact.phone }} | {{ contact.email }} | {{ contact.company }} | {{ message.text }} | {{ trigger.event }}'
      ),
    ];
    ...
    expect(h.state.upsertCalls).toHaveLength(1);
    expect((h.state.upsertCalls[0].payload as { value: string }).value).toBe(
      'Ana Silva | +351900000000 | ana@example.com | Lumiar | Olá | new_message_received'
    );
  });
