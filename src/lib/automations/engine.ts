...
type AutomationInterpolationContact = {
  id: string;
  name?: string | null;
  phone?: string | null;
  email?: string | null;
...
export async function resumePendingExecution(pending: {
  ...
  try {
    const contact = pending.contact_id
      ? await loadInterpolationContact(
          db,
...
async function executeAutomation(automation: Automation, input: DispatchInput) {
  ...
  const contact = input.contactId
    ? await loadInterpolationContact(db, automation.account_id, input.contactId)
    : null;
...
interface ExecuteArgs {
  ...
  contactId: string | null;
  contact: AutomationInterpolationContact | null;
  context: AutomationContext;
...
async function loadInterpolationContact(
  db: ReturnType<typeof supabaseAdmin>,
  accountId: string,
  contactId: string
): Promise<AutomationInterpolationContact | null> {
  ...
  if (error) {
    console.error('[automations] contact interpolation lookup failed:', error);
    return null;
  }

  return (data as AutomationInterpolationContact | null) ?? null;
}
...
async function runStep(
  ...
  switch (step.step_type) {
    case 'send_message': {
      ...
      return `sent via QR (${whatsapp_message_id})`;
    }
    ...
    case 'send_list': {
      ...
      return `interactive sent via QR (${whatsapp_message_id})`;
    }
    ...
    case 'send_template': {
      ...
      return `template sent via QR (${whatsapp_message_id})`;
    }
...
function interpolate(s: string, args: ExecuteArgs): string {
  return s.replace(/\{\{\s*([\w.]+)\s*\}\}/g, (_, key) => {
    ...
    if (ns === 'contact' && prop) {
      const value =
        args.contact?.[prop as keyof AutomationInterpolationContact];
      return value == null ? '' : String(value);
    }
    if (ns === 'conversation' && prop === 'id') {
      return String(args.context.conversation_id ?? '');
    }
    if (ns === 'automation' && prop) {
      const value = args.automation[prop as keyof Automation];
      return value == null ? '' : String(value);
    }
    if (ns === 'trigger' && prop === 'event') return args.triggerEvent;
    if (ns === 'tag' && prop === 'id') return String(args.context.tag_id ?? '');
    if (ns === 'agent' && prop === 'id')
      return String(args.context.agent_id ?? '');
    if (ns === 'reply' && prop === 'id') {
      return String(args.context.interactive_reply_id ?? '');
    }
    return '';
  });
}
