...
/**
 * QR sessions flatten interactive buttons/lists into a numbered text menu.
 * When the customer replies with "1", "2", the visible option title, or the
 * internal reply_id, route that text to the same next node a native Meta tap
 * would have selected.
 */
export function matchTextOption(
  node: { node_type: string; config: Record<string, unknown> },
  text: string
): string | null {
  ...
  const matchesOption = (
    option: { reply_id?: string; title?: string; next_node_key?: string },
    index: number
  ) => {
    const numeric = String(index + 1);
    const replyId = option.reply_id?.trim().toLowerCase();
    const title = option.title?.trim().toLowerCase();
    return (
      normalized === numeric || normalized === replyId || normalized === title
    );
  };

  if (node.node_type === 'send_buttons') {
    const cfg = node.config as unknown as SendButtonsNodeConfig;
    const hit = cfg.buttons?.find((button, index) =>
      matchesOption(button, index)
    );
    return hit?.next_node_key ?? null;
  }
  ...
  if (node.node_type === 'send_list') {
    ...
    for (const section of cfg.sections ?? []) {
      for (const row of section.rows ?? []) {
        if (matchesOption(row, index)) return row.next_node_key;
        index += 1;
      }
    }
  }
...
async function handleReplyForActiveRun(
  ...
  if (
    ...
    matched = matchTextOption(currentNode, message.text);
  } else if (
    message.kind === 'text' &&
