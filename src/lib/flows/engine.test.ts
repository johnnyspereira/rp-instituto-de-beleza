...
describe('matchTextOption', () => {
  it('matches QR-style numeric replies for buttons', () => {
    const node = {
      node_type: 'send_buttons',
      config: {
        buttons: [
    ...
    expect(matchTextOption(node, '1')).toBe('to_sales');
    expect(matchTextOption(node, '2')).toBe('to_support');
  });

  it('matches option titles and reply ids case-insensitively', () => {
    const node = {
      node_type: 'send_buttons',
      config: {
        buttons: [
          {
            reply_id: 'quote',
            title: 'Get quote',
            next_node_key: 'quote_flow',
          },
        ],
      },
    };

    expect(matchTextOption(node, 'GET QUOTE')).toBe('quote_flow');
    expect(matchTextOption(node, 'quote')).toBe('quote_flow');
  });

  it('matches QR-style numeric replies across list sections', () => {
    const node = {
      node_type: 'send_list',
      config: {
        sections: [
          ...
          {
            rows: [{ reply_id: 'c', title: 'C', next_node_key: 'node_c' }],
          },
        ],
      },
    };

    expect(matchTextOption(node, '3')).toBe('node_c');
    expect(matchTextOption(node, 'c')).toBe('node_c');
    expect(matchTextOption(node, 'unknown')).toBeNull();
  });
});
