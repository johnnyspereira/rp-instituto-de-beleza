...
export default function BroadcastDetailPage() {
  ...
  const isQrBroadcast =
    broadcast.template_language === 'internal' ||
    broadcast.audience_filter?.templateSource === 'internal';
  ...
  return (
    <div className="space-y-6">
      ...
      {isQrBroadcast && (
        <div className="flex gap-3 rounded-xl border border-amber-500/25 bg-amber-500/10 p-4 text-sm">
          <AlertCircle className="mt-0.5 h-4 w-4 shrink-0 text-amber-500" />
          <div>
            <p className="text-foreground font-medium">{t('qrNoticeTitle')}</p>
            <p className="text-muted-foreground mt-1">
              {t('qrNoticeDescription')}
            </p>
          </div>
        </div>
      )}
                ...
                {filteredRecipients.map((recipient) => {
                  ...
                  return (
                    <TableRow key={recipient.id} className="border-border">
                      ...
                      <TableCell
                        className="max-w-sm text-xs whitespace-normal text-red-400"
