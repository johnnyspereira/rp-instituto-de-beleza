...
const { disconnectedQrStatus } = vi.hoisted(() => ({
  disconnectedQrStatus: {
    connected: false,
    state: 'idle' as const,
    qr: null,
    lastError: null,
...
vi.mock('@/lib/whatsapp/baileys', () => ({
  getBaileysSessionStatus: vi.fn(async () => disconnectedQrStatus),
  startBaileysSession: vi.fn(async () => disconnectedQrStatus),
  sendTextViaBaileys: vi.fn(),
  sendMessageViaBaileys: vi.fn(),
}));
