...
export function useBroadcastSending(): UseBroadcastSendingReturn {
  ...
  async function createAndSendBroadcast(
    ...
    try {
      ...
      const isInternalTemplate = isInternalBroadcastTemplate(payload.template);
      ...
      const { data: broadcast, error: broadcastError } = await supabase
        ...
        .insert({
          ...
          template_language: isInternalTemplate
            ? 'internal'
          ...
          audience_filter: {
            ...
            templateSource: isInternalTemplate ? 'internal' : 'meta',
          },
          status: 'sending',
      ...
      for (let i = 0; i < recipients.length; i += SEND_BATCH_SIZE) {
        ...
        const apiRecipients = batch
          ...
          .map((r) => {
            const contact = r.contact as Contact;
            if (isInternalTemplate) {
              return {
                phone: contact.phone,
                contact_id: contact.id,
                content_text: renderInternalBroadcastText(
                  payload.template.body_text,
                  contact
                ),
                interactive_payload: payload.template.interactive_payload,
              };
            }
        ...
        try {
            ...
            body: JSON.stringify(
              isInternalTemplate
                ? {
                    ...
                    internal_kind: payload.template.internal_kind ?? 'text',
                  }
                : {
                    recipients: apiRecipients,
                    template_name: payload.template.name,
          ...
          for (const recipient of batch) {
            ...
            const result =
              contactId && resultsByContactId.has(contactId)
